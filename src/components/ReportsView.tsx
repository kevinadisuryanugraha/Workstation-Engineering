  ...
  const getProjectShowcase = () => {
    if (project.key === "LMS" || project.id === "proj-lms") {
      ...
      return {
        ...
        techDetail: "Resolved raw socket write deadlock on Kontabo proxy daemon by inserting a 2500ms timeout guard, draining memory buffers on socket close, and releasing hotfix patch v2.8.1 to cashier nodes.",
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white rounded-xl border border-slate-200/90 shadow-sm p-6 sm:p-8 space-y-7">
        ...
        {/* Document Footer */}
        <div className="border-t border-slate-100 pt-4 flex flex-col sm:flex-row sm:items-center justify-between text-[11px] text-slate-400 font-mono gap-2">
