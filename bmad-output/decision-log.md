...
- **Impact:** total 65 story (23 epic) semua DONE — 100%; test suite bertambah menjadi 433/433 test passed across 59 files; migrasi DB 0017; nav tidak bertambah (kemenangan CC-4 dijaga).
- **In-progress stories affected:** none.
- **Made by:** bmad-epic-pipeline-worktree (dev loop) → review → gate → finalize
- **Supersedes:** none (penutup CC-8)

---

### 2026-09-22 — Course Correction 8: Tambah Epic 23 — Multi-Provider Git (GitLab & Bitbucket)
- **Decision:** Owner memilih Opsi 1 dari 4 kandidat sisa Fase V2 (Master PRD §30) — mengeksekusi item **"multi-provider Git"** yang sejak Epic-5 ditunda eksplisit ("Multi-provider Git selain GitHub ditunda ke V2") dan terverifikasi tidak pernah dibahas di decision-log/epics/PRD. **Epic 23** (4 story): **23.1** Migration 0017 (kolom `provider` pada `repositories` & `webhook_deliveries`, default GITHUB) + Repository Registry API idempoten (GET auth-only / POST `PERM_EVIDENCE_ATTACH`, secret server-generated, tidak pernah ke client); **23.2** webhook adapter GitLab (`X-Gitlab-Token` timing-safe) & Bitbucket (`X-Hub-Signature` HMAC-SHA256) dengan normalisasi payload ke model kanonik + delivery ID fallback deterministik untuk GitLab tanpa `X-Gitlab-Event-UUID`; **23.3** pemrosesan kanonik → tabel commits/pull_requests + evidence links regex TIDAK berubah, `REPO_NOT_REGISTERED` → FAILED jujur, read API 18.1 menyertakan `provider`; **23.4** UI badge provider + form registrasi repo di panel Git eksisting (nav TIDAK bertambah). Alternatif yang ditolak: menambah permission RBAC baru (mengubah matriks 9×20 — pola 22.1 menolak), refactor handler GitHub ke generik penuh (risiko regression ADR-005 — dual-path diperbolehkan bila regression berisiko).
- **Rationale:** Sisa Fase V2 yang tersisa (multi-provider Git, CI/CD adapters, advanced deployment gates, advanced analytics) kini menjadi frontier resmi; multi-provider Git dipilih karena paling selaras dengan inti produk — Git Intelligence adalah jantung traceability WORKSTATION (Tiket → Work Item → Commit → MR → Deployment), dan mayoritas alurnya sudah provider-agnostic (regex kode item, evidence links); yang GitHub-specific hanya lapisan tipis header/payload — pas untuk satu epic.
- **Impact:** epics.md (+Epic 23, total 65 story: done 61, remaining 4, 94%; wave plan +28–31); stories/ (+4 file ready-for-dev); sprint-status.yaml (+epic-23 in-progress, parallel_set 28–31 sequential — shared `git` module, `webhook.service.ts`, `App.tsx`); project-context.md (catatan CC-8).
- **In-progress stories affected:** none (61 story lama tetap done).
- **Made by:** bmad-correct-course
- **Supersedes:** none (additive — membuka sisa Fase V2 pasca penutupan CC-7)
